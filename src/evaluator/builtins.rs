...
impl BuiltinFunctions {
    ...
    pub fn get_builtins(&self) -> Vec<(Ident, Object)> {
        vec![
            add_builtin("println", 1, bprintln_fn),
...
fn add_builtin(name: &str, arity: usize, func: BuiltinFunction) -> (Ident, Object) {
    let name = name.to_owned();
    (Ident(name.clone()), Object::Builtin(name, arity, func))
}

fn bprintln_fn(args: Vec<Object>) -> Result<Object, String> {
    match args.get(0) {
...
fn blen_fn(args: Vec<Object>) -> Result<Object, String> {
    match args.get(0) {
        Some(Object::String(s)) => Ok(Object::Number(s.len() as i64)),
        Some(Object::Array(arr)) => Ok(Object::Number(arr.len() as i64)),
